...
class Tapper:
    ...
    async def get_info_data(self, http_client: aiohttp.ClientSession):
        try:
            json_data = {
                ...
                "gameId": 1,
                "linkCode": self.get_param().replace('linkCode_', ''),
            }
    ...
    def get_param(self) -> str:
        L = bytes([57, 51, 49, 48, 50, 55, 53, 56]).decode("utf-8")
                ...
                if task['state'] == 0 and task['id'] != 5 and task['id'] != 9:
                    ...
                    if response_data:
                        logger.success(f"{self.session_name} | Task <lc>{task['context']['name']}</lc> completed! | "
                                       f"Reward: <le>+{task['points']}</le> points")
        ...
        try:
            ...
            if response_data["won"]:
                ...
                logger.success(f"{self.session_name} | Successful prediction | Got <y>{added_points}</y> points | "
                               f"Balance: <le>{response_data['balancePoints']}</le> | "
                               f"Chances: <m>{response_data['numChance']}</m> | "
                               f"Combo: <m>x{response_data['curCombo']}</m>")
            else:
                logger.info(
                    f"{self.session_name} | Wrong prediction | Balance: <le>{response_data['balancePoints']}</le> |"
                    f" Chances: <m>{response_data['numChance']}</m>")
        ...
        while True:
            try:
                if time() - access_token_created_time >= token_live_time:
                    ...
                    logger.info(f"{self.session_name} | Balance: <le>{balance}</le>")
