import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from global_data.global_config import global_settings
from scripts.file_manager import load_from_json, save_to_json
from scripts.logger import logger
from scripts.agents import generate_random_user_agent


class Accounts:
    def __init__(self):
        self.workdir = "global_data/sessions/"
        self.api_id = global_settings.API_ID
        self.api_hash = global_settings.API_HASH

    @staticmethod
    def get_available_accounts(sessions: list):

        accounts_from_json = load_from_json('global_data/sessions/accounts.json')

        if not accounts_from_json:
            raise ValueError("Can't run script | Please, add account/s in global_data/sessions/accounts.json")

        available_accounts = []
        for session in sessions:
            is_session_added = False
            for saved_account in accounts_from_json:
                if saved_account['session_name'] == session:
                    available_accounts.append(saved_account)
                    is_session_added = True
                    break
            if not is_session_added:
                logger.warning(f'{session}.session does not exist in global_data/sessions/accounts.json')
                ans = input(f"Add {session} to accounts.json? (y/N): ")
                if 'y' in ans.lower():
                    raw_proxy = input("Input the proxy in the format type://user:pass:ip:port (press Enter to use without proxy): ")
                    user_agent = input("Input the user agent: (press Enter to use random user agent): ")
                    new_account = {
                         "session_name": session,
                        "user_agent": user_agent if user_agent else generate_random_user_agent(device_type='android', browser_type='chrome'),
                        "proxy": raw_proxy if raw_proxy else None
                    }
                    save_to_json(f'global_data/sessions/accounts.json', dict_=new_account)
                    available_accounts.append(new_account)
                    logger.success(f'Account {session} added successfully')

        return available_accounts

    def pars_sessions(self):
        sessions = []
        for file in os.listdir(self.workdir):
            if file.endswith(".session"):
                sessions.append(file.replace(".session", ""))

        # logger.info(f"Searched sessions: {len(sessions)}.")
        return sessions

    async def get_accounts(self):
        sessions = self.pars_sessions()
        available_accounts = self.get_available_accounts(sessions)

        if not available_accounts:
            raise ValueError("You don't have any available accounts!")
        else:
            logger.info(f"Available accounts: {len(available_accounts)}")

        return available_accounts



    @staticmethod
    def get_ready_accounts(sessions: list):

        accounts_from_json = load_from_json('global_data/sessions/accounts.json')

        if not accounts_from_json:
            raise ValueError("Can't run script | Please, add account/s in global_data/sessions/accounts.json")
        
        ready_accounts = []
        for session in sessions:
            is_session_added = False
            for saved_account in accounts_from_json:
                if saved_account['session_name'] == session:
                    ready_accounts.append(saved_account)
                    is_session_added = True
                    break

        return ready_accounts
    

    async def ready_accounts(self):
        sessions = self.pars_sessions()
        ready_accounts = self.get_available_accounts(sessions)

        if not ready_accounts:
            raise ValueError("You don't have any available accounts!")
        # else:
        #     logger.success(f"Available accounts: {len(ready_accounts)}.")

        return ready_accounts
    

